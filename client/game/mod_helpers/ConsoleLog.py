...
class ConsoleLogManager:
    ...
    def auto_color_text(self, text: str) -> str:
        ...
        if not is_server_msg:
            ...
            if ConsoleLogManager._items_cache is None:
                try:
                    from .MapMenu import items_shortened
    ...
    def on_print_silently(self, text: str, rich_data: list = None, html_text: str = None, mirror_to_hud: bool = False, from_logger: bool = False):
        ...
        noise_filters = ["changed tags from", "now that you are connected", 
        ...
        if not from_logger:
            if mirror_to_hud:
                if is_death_event:
                    logger.info(raw_text, extra={"from_sync": True})
                else:
                    logger.info(f"[HUD] {raw_text}", extra={"from_sync": True})
            else:
                if not any(x in raw_text for x in ["[Archipelago]", "[HUD]", "DEATHLINK:", "Connection to Portal 2", "Disconnected from Portal 2"]):
                    logger.info(f"[Archipelago] {raw_text}", extra={"from_sync": True})
                else:
                    logger.info(raw_text, extra={"from_sync": True})
        
        logger.debug(f"Notification: {raw_text}")
