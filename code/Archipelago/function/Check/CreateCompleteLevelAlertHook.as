// =============================================================
// ARCHIPELAGO CREATE COMPLETE LEVEL ALERT HOOK
// =============================================================
void CreateCompleteLevelAlertHook(string map) {
    g_has_printed_map_complete = false;
    
    // Initialisation du compteur pour les maps Ã  double trigger
    if (two_trigger_levels.find(map) >= 0) {
        transition_script_count = 1;
    }

    // --- LE SCAN DES TRIGGERS ANONYMES (La clÃ© qui remplace le VScript) ---
    CBaseEntity@ tr = null;
    ...
    // --- LOGIQUE NON-ELEVATOR (MÃ©thode SimplifiÃ©e) ---
    else if (non_elevator_maps.find(map) >= 0) {
        
        // LA MÃ‰THODE "BTS5" APPLIQUÃ‰E PARTOUT : 
        // On dÃ©truit proprement le script de transition natif pour TOUTES ces maps.
        // Cela empÃªche le jeu de faire un "hot-swap" du niveau sous nos pieds.
