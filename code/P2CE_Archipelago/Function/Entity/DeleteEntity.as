...
void DeleteEntity(const string&in entity_name, bool create_holo = true) {
    ...
    // Nouvelles exceptions sp_a2_bts4
    ...
    for (uint i = 0; i < entsToDelete.length(); i++) {
        CBaseEntity@ ent = entsToDelete.opIndex(i);
        ...
        if (create_holo) {
            ...
            // --- CALCUL standard de la position/rotation de l'holo ---
            ...
            // --- MODIFICATION EXCEPTION DIRECTE DES LASERS (Bypass complet de l'override standard) ---
            ...
            if (classname.locate("env_portal_laser") != uint(-1) || 
                ...
                // 1. Calcul de la position mondiale absolue voulue
                if (classname.locate("prop_laser_relay") != uint(-1)) {
                    finalPos = spawnPos + (up * 32.0f); // Décale vers le haut local du relais
                } else {
                    finalPos = spawnPos + (forward * 32.0f); // Avance dans l'axe du faisceau
                }

                // 2. Calcul de la rotation mondiale absolue voulue
                if (classname.locate("prop_laser_relay") != uint(-1)) {
                    finalAng = angles; // Reste aligné sur le relais
                } else {
                    // Recalcule l'orientation pour faire face au faisceau proprement
