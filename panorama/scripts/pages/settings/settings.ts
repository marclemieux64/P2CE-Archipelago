...
class MainMenuSettings {
    ...
    static navigateToTab(tab) {
        ...
        if (this.activeTab !== tab) {
            ...
            if (this.activeTab) {
                // Hide the active tab
                const tabElement = $.GetContextPanel().FindChildInLayoutFile(this.activeTab);
                tabElement?.RemoveClass('settings-page--active');
            }
    ...
    static initPersistentStorageEnum(panel, storageKey) {
        const convar = panel.convar || '';
        const storedValue = $.persistentStorage.getItem(storageKey) ?? 0;

        if (convar) {
            GameInterfaceAPI.ConsoleCommand(`${convar} ${storedValue}`);
        }
        ...
        for (const child of panel.FindChildTraverse('values').Children()) {
            ...
            child.checked = (parseInt(String(storedValue), 10) === value);

            const overrideString = child.GetAttributeString('activateoverride', '');

            let activateFn: Function | null = null;
            if (overrideString) {
                try {
                    activateFn = new Function(overrideString);
                } catch (e) {
                    $.Warning(`Failed to compile activateoverride snippet: ${overrideString}`);
                }
            }
            ...
            child.SetPanelEvent('onactivate', () => {
                ...
                if (convar) {
                    GameInterfaceAPI.ConsoleCommand(`${convar} ${value}`);
                }
                if (activateFn) {
                    try {
                        activateFn();
                    } catch (e) {
                        $.Warning("activateoverride execution deferred or failed cleanly during UI interactions.");
                    }
                }
            });
        }
    ...
    static initPersistentStorageEnumDropdown(panel, storageKey) {
        ...
        const rawStore = $.persistentStorage.getItem(storageKey);
        const targetIndex = rawStore !== null ? parseInt(rawStore, 10) : 0;
        dropdown.SetSelectedIndex(targetIndex);
        ...
        if (overrideString) {
            try {
                submitFn = new Function(overrideString);
            } catch (e) {
                $.Warning(`Failed to compile submitoverride snippet: ${overrideString}`);
            }
        }
